...
// ==========================================
// SCENE OPTIONS (Programmatic Randomness)
// ==========================================
const SCENE_OPTIONS = [
  "A runner briefly paused from fatigue, adjusting gear",
...
## 2. SCENE SELECTION (MANDATORY)
**You must generate the prompt for the following specific moment:**
**"{SELECTED_SCENE}"**

Do not deviate from this scene. Do not combine it with other concepts.
Avoid "runner mid-stride hero" compositions.
...
Deno.serve(async (req) => {
  ...
  try {
    // ------------------------------------------
    // STEP 0: SELECT RANDOM SCENE (Programmatically)
    // ------------------------------------------
    const randomScene = SCENE_OPTIONS[Math.floor(Math.random() * SCENE_OPTIONS.length)];
    console.log(`[Queue] Selected Scene: ${randomScene}`);

    // ------------------------------------------
    // STEP 1: MISTRAL PROMPT GENERATION
    // ------------------------------------------
    // Inject both the Event Name AND the Randomly Selected Scene
    const formattedPrompt = MASTER_SYSTEM_PROMPT
      .replace("{EVENT_NAME}", festivalName)
      .replace("{SELECTED_SCENE}", randomScene);
